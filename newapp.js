...
var privateKey  = fs.readFileSync('./certificate/lovelu.key', 'utf8');
var certificate = fs.readFileSync('./certificate/lovelu.pem','utf8');
...
function getFeed(db, openId, typeA, idA){
    // 目前返回like数最多的，然后shuffle
    return new Promise(function(resolve, reject){
        let col = db.collection('article_collection')
        let items = []
        col.find({}).sort({like:-1, dislike:1}).limit(50).project({'text':0}).toArray().then(function(result){
            choosedItems = shuffleArray(result).slice(0,5);
            for (choosedItem of choosedItems){
                items.push({title:choosedItem.title, ref:{type:'article', params:{id:choosedItem.a_id}}});
            }
            resolve(items)
            return;
...
app.get('/QA', function(req, res){
    if(req.protocol === 'https'){
        ...
        if (reqType==='query') {
            ...
            res.status(500).send({'err_msg':'NO SUCH TYPE!!!'})
        }
        // res.status(200).send({status:"200",type:"item",content:req.query.params.text});
    } else {
        res.status(500).send({'err_msg':"SHOULD BE HTTPS!!!"})
    }
    return
});
    ...
    if(req.protocol === 'https'){
        ...
        if (reqType === 'article'){
            ...
            mongoClient.connect(mongoURL, function(err,client){
                ...
                Promise.all([getArticle(),getLike(db, reqOpenId, reqType, reqParams.id), getFeed(db, reqOpenId, reqType, reqParams.id)])
                .then(function(values){
                    ...
                    out_data.content.feed = values[2];
                    console.log(out_data)
            ...
            mongoClient.connect(mongoURL, function(err,client){
                ...
                Promise.all([getItem(),getLike(db, reqOpenId, reqType, reqParams.id), getFeed(db, reqOpenId, reqType, reqParams.id)])
                .then(function(values){
                    console.log(SendRequest)
                    let out_data = {type:'article', content:{}}
                    out_data.content.title = values[0].title;
...
app.get('/dinnerinit', function(req, res){
    if(req.protocol === 'https'){
        let reqParams = JSON.parse(req.query.params)
	let myurl = "https://api.weixin.qq.com/sns/jscode2session?appid=wxc8cf1390df439aab&secret=060102c12851fe2846a2a37dfce9c4f9"
        let url = myurl + '&js_code='+reqParams.JSCODE + '&grant_type=authorization_code'
        https.get(url, (result) => {
                console.log('statusCode:', result.statusCode);
                console.log('headers:', result.headers);
                result.on('data', (d) => {
                    ...
                    //connection.connect();
                    let default_query="select food_name from default_food order by id";
                    connection.query(default_query,function (err, result) {
			if(err){
          		     console.log('default_food selected error - ',err.message);
                             res.status(500).send({err_msg: err.message});
			}
			else{
			    res.status(200).send({open_id:data.openid,default_food:result})
			}	
                    });
                    //res.status(200).send({open_id:data.openid})
                });
